...
import {
  ArrowLeft, Clock, Download, ExternalLink,
  FileText, Pencil, Upload, X, User, Calendar,
...
const DARK = {
  bg:"#0d0f18", card:"#1a1d2e", border:"#252840",
  text:"#e2e8f0", muted:"#8892a4", hint:"#55607a",
  input:"#1e2235", topbar:"#12151f",
};
const LIGHT = {
  bg:"#f4f6fb", card:"#ffffff", border:"#e8edf3",
  text:"#111827", muted:"#64748b", hint:"#94a3b8",
  input:"#f8fafc", topbar:"#ffffff",
};
  ...
  const savePage = async () => {
    ...
    try {
      ...
      await axios.put(`http://192.168.11.69:5000/pages/${pageId}`,
        { title: nextTitle, content: editContent },
        { headers: authHeaders() }
      );
      setPage(prev => prev ? { ...prev, title: nextTitle, content: editContent } : prev);
          ...
          <div style={{ background:C.card, border:`1px solid ${C.border}`, borderRadius:"20px", padding:"32px 36px", marginBottom:"20px" }}>
            ...
            <div style={{ display:"flex", flexWrap:"wrap", alignItems:"center", justifyContent:"center", gap:"16px", color:C.muted, fontSize:"13px", paddingTop:"16px", borderTop:`1px solid ${C.border}` }}>
              <span style={{ display:"flex", alignItems:"center", gap:"5px" }}>
                <User size={13}/>
                {page?.creator_name ? `By ${page.creator_name}` : "OneSpace User"}
              </span>
              ...
              {page?.updated_at && page.updated_at !== page.created_at && (
                <>
                  ...
                  <span style={{ display:"flex", alignItems:"center", gap:"5px" }}>
                    <Clock size={13}/>
                    Edited {formatDate(page?.updated_at)}
                  </span>
                </>
              )}
              <span style={{ color:C.hint }}>•</span>
            ...
            {attachOpen && (
              <div style={{ padding:"16px" }}>
                ...
                <div style={{ marginBottom:"12px" }}>
                  <div style={{ border:`1.5px dashed ${C.border}`, borderRadius:"12px", padding:"12px", textAlign:"center", cursor:"pointer", transition:"border-color 0.2s" }}
                ...
                <div style={{ display:"flex", flexDirection:"column", gap:"6px" }}>
                  {attachments.length === 0 ? (
                    <div style={{ textAlign:"center", padding:"16px 0", color:C.hint, fontSize:"12px" }}>
                      No attachments yet
                    </div>
                  ) : (
                    attachments.map(file => (
                      <div key={file.id} style={{ background:isDark?"rgba(30,34,53,0.6)":"#fafafa", border:`1px solid ${C.border}`, borderRadius:"10px", padding:"10px 12px" }}>
                        <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"6px" }}>
                          ...
                          <div style={{ flex:1, minWidth:0 }}>
                            <p style={{ color:C.text, fontSize:"12px", fontWeight:600, margin:0, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>
                              {file.file_name}
                            </p>
