...
<!DOCTYPE html><html><body style="margin:0;padding:0;background:#0d0f18;font-family:sans-serif;">
<div style="max-width:560px;margin:40px auto;background:#12151f;border:1px solid #252840;border-radius:16px;overflow:hidden;">
  ...
  <div style="padding:32px;">
    ...
    <p style="color:#8892a4;font-size:14px;line-height:1.6;">Please verify your email address to activate your account.</p>
    <div style="text-align:center;margin:32px 0;">
      <a href="${verifyUrl}" style="background:linear-gradient(135deg,#4f46e5,#7c3aed);color:white;text-decoration:none;padding:14px 32px;border-radius:12px;font-weight:600;font-size:15px;display:inline-block;">Verify Email Address</a>
    </div>
    <p style="color:#55607a;font-size:12px;text-align:center;">This link expires in 24 hours.</p>
  </div>
</div></body></html>`;
...
<div style="max-width:560px;margin:40px auto;background:#12151f;border:1px solid #252840;border-radius:16px;overflow:hidden;">
  ...
  <div style="padding:32px;">
    ...
    <p style="color:#8892a4;font-size:14px;line-height:1.6;">Invited by <strong style="color:#e2e8f0;">${inviterName}</strong>.</p>
    <div style="text-align:center;margin:32px 0;">
      <a href="${registerUrl}" style="background:linear-gradient(135deg,#4f46e5,#7c3aed);color:white;text-decoration:none;padding:14px 32px;border-radius:12px;font-weight:600;font-size:15px;display:inline-block;">Accept Invitation & Register</a>
    </div>
  </div>
</div></body></html>`;
...
<div style="max-width:560px;margin:40px auto;background:#12151f;border:1px solid #252840;border-radius:16px;overflow:hidden;">
  ...
  <div style="padding:32px;">
    <p style="color:#e2e8f0;font-size:16px;">Hi <strong>${username}</strong>, you've been added to <strong style="color:#a5b4fc;">${spaceName}</strong>.</p>
    <div style="text-align:center;margin:32px 0;">
      <a href="${dashboardUrl}" style="background:linear-gradient(135deg,#4f46e5,#7c3aed);color:white;text-decoration:none;padding:14px 32px;border-radius:12px;font-weight:600;font-size:15px;display:inline-block;">Open Dashboard</a>
    </div>
  </div>
</div></body></html>`;
...
const adminOnly = async (req, res, next) => {
  try {
    ...
    if (userRes.rows[0]?.role !== "admin") return res.status(403).json({ error: "Admin access required" });
    next();
...
async function checkWorkspaceRole(userId, workspaceId, allowedRoles) {
  ...
  if (directRes.rows.length > 0) return allowedRoles.includes(directRes.rows[0].role);
  const teamRes = await pool.query(
    `SELECT tw.permission as role FROM team_members tm
     JOIN team_workspaces tw ON tm.team_id = tw.team_id
     WHERE tm.user_id = $1 AND tw.workspace_id = $2 LIMIT 1`,
    [userId, workspaceId]
  );
  if (teamRes.rows.length > 0) return allowedRoles.includes(teamRes.rows[0].role);
  return false;
}
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO users (username, email, password_hash, role, verified, verification_token, verification_expires)
       VALUES ($1, $2, $3, 'user', FALSE, $4, $5) RETURNING id, username, email, role`,
    ...
    const emailSent = await sendEmail(email, "Verify your OneSpace account", verificationEmailHTML(username, verifyUrl));
    const pendingInvites = await pool.query("SELECT * FROM pending_invites WHERE email = $1", [email]);
...
app.post("/resend-verification", async (req, res) => {
  try {
    ...
    const result = await pool.query("SELECT id, username, verified FROM users WHERE email = $1", [email]);
  ...
  try {
    ...
    if (isAdmin) {
      ...
      result = await pool.query(
        `SELECT DISTINCT w.* FROM workspaces w WHERE w.id IN (
          SELECT workspace_id FROM workspace_members WHERE user_id = $1
          UNION SELECT tw.workspace_id FROM team_members tm
          JOIN team_workspaces tw ON tm.team_id = tw.team_id WHERE tm.user_id = $1
...
app.get("/workspaces/:id/members", authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `SELECT u.id, u.username, u.email, wm.role, wm.created_at FROM workspace_members wm
       JOIN users u ON wm.user_id = u.id WHERE wm.workspace_id = $1 ORDER BY wm.created_at ASC`,
      [req.params.id]
    );
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO workspace_members (workspace_id, user_id, role) VALUES ($1, $2, $3)
       ON CONFLICT (workspace_id, user_id) DO UPDATE SET role = $3 RETURNING *`,
...
app.post("/workspaces/:id/invite", authenticateToken, async (req, res) => {
  try {
    ...
    await pool.query(
      `INSERT INTO pending_invites (email, workspace_id, role, token, invited_by) VALUES ($1, $2, $3, $4, $5)
       ON CONFLICT (email, workspace_id) DO UPDATE SET role = $3, token = $4`,
    ...
    await sendEmail(email, `You've been invited to ${spaceName} on OneSpace`, inviteEmailHTML(inviterName, spaceName, registerUrl));
...
app.delete("/pages/:id", authenticateToken, async (req, res) => {
  try {
    ...
    if (pageResult.rows.length === 0) return res.status(404).json({ error: "Page not found" });
    const allowed = await checkWorkspaceRole(req.user.id, pageResult.rows[0].workspace_id, ["admin", "editor"]);
    if (!allowed) return res.status(403).json({ error: "No permission to delete this page" });
    await pool.query("DELETE FROM pages WHERE id = $1", [req.params.id]);
    res.json({ message: "Page deleted", workspace_id: pageResult.rows[0].workspace_id });
...
app.post("/teams/:id/workspaces", authenticateToken, async (req, res) => {
  try {
    ...
    const result = await pool.query(
      `INSERT INTO team_workspaces (team_id, workspace_id, permission, granted_by) VALUES ($1, $2, $3, $4)
       ON CONFLICT (team_id, workspace_id) DO UPDATE SET permission = $3 RETURNING *`,
...
app.get("/teams/:id/workspaces", authenticateToken, async (req, res) => {
  try {
    const result = await pool.query(
      `SELECT w.id, w.name, w.description, tw.permission FROM team_workspaces tw
       JOIN workspaces w ON tw.workspace_id = w.id WHERE tw.team_id = $1`, [req.params.id]
    );
    res.json(result.rows);
  } catch (err) { res.status(500).json({ error: err.message }); }
});
...
app.delete("/teams/:id", authenticateToken, async (req, res) => {
  try {
    await pool.query("DELETE FROM teams WHERE id = $1 AND created_by = $2", [req.params.id, req.user.id]);
...
app.get("/admin/stats", authenticateToken, adminOnly, async (req, res) => {
  try {
    ...
    const spaces = await pool.query("SELECT COUNT(*) FROM workspaces");
    const pages = await pool.query("SELECT COUNT(*) FROM pages");
    const unverified = await pool.query("SELECT COUNT(*) FROM users WHERE verified = FALSE");
    res.json({
      users: parseInt(users.rows[0].count),
      spaces: parseInt(spaces.rows[0].count),
  ...
  try {
    const result = await pool.query(
      `SELECT u.id, u.username, u.email, u.role, u.verified, u.created_at,
        (SELECT COUNT(*) FROM workspace_members wm WHERE wm.user_id = u.id) as space_count
...
app.put("/admin/users/:id", authenticateToken, adminOnly, async (req, res) => {
  try {
    ...
    if (verified !== undefined) { updates.push(`verified = $${idx++}`); values.push(verified); }
